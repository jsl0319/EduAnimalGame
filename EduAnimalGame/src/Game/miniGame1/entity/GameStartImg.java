...
public class GameStartImg {
	...
	public GameStartImg() {
		try {
			img = ImageIO.read(new File("res/img/miniGame1Canvas/gameStartImg.png"));
		} catch (IOException e) {
...
//		y = (MiniGame1Canvas.instance.getHeight()-height)/2;
	...
	public void paint(Graphics g) {
		...
		int imgIdx = MiniGame1Canvas.getStartIdx();
		if(imgIdx<=2) {
			int x = (int) this.x;
			int y = (int) this.y;
			int width = (int) this.width;
			int height = (int) this.height;
			g.drawImage(img, x, y, x+width, y+height, imgIdx*width, 0, imgIdx*width+width, height, MiniGame1Canvas.instance);
